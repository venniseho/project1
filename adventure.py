...
def check_victory(p: Player) -> None:
    ...
    >>> p = Player(5, 4)
    >>> Cheat = Item("Cheat Sheet", 3, 12, 10)
    >>> Lucky = Item("Lucky Pen", 1, 12, 10)
    >>> Card = Item("T Card", 6, 12, 10)
    >>> p.inventory.append(Cheat)
    >>> p.inventory.append(Lucky)
    >>> check_victory(p)
    >>> p.victory
    False
    >>> p.inventory.append(Card)
    >>> check_victory(p)
    >>> p.victory
    True
    """
